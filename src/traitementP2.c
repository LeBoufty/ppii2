    ...
    while (currenttrajet->next!=NULL){//Parcours tous les utilisateurs
        if (currenttrajet->info->Nb_ticks_attente<=0){//Si l'utilisateur est à une borne
            if (currenttrajet->info->ID_courrant==0){//Si l'utilisateur est à la fin du trajet
                //destroy trajets
                add_station_tab_nbre_pdc_dispo(currenttrajet->info->chemin->s_tab,get_station_tab_id(currenttrajet->info->chemin->s_tab,currenttrajet->info->ID_courrant), 1);//EUHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
            }
                ...
                if (get_station_tab_nbre_pdc_dispo(currenttrajet->info->chemin->s_tab,get_station_tab_id(currenttrajet->info->chemin->s_tab,currenttrajet->info->ID_courrant))>0){//Regarde si il y a de la place dans la station
                    currenttrajet->info->Nb_ticks_attente=-get_chemin_tab_struct_temps_recharge(currenttrajet->info->chemin,currenttrajet->info->ID_courrant)/ticksparh+1;//Initialise le rechargement
                    add_station_tab_nbre_pdc_dispo(currenttrajet->info->chemin->s_tab,get_station_tab_id(currenttrajet->info->chemin->s_tab,currenttrajet->info->ID_courrant), -1)//Réserve sa place dans la station
                }
        }
